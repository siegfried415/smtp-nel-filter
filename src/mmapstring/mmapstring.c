...
static void
__mutex_lock (CRITICAL_SECTION * section)
{
...
static void
__mutex_unlock (CRITICAL_SECTION * section)
{
	if (!_lock_initialized)
		InitializeCriticalSection (section);
...
void
mmap_string_set_tmpdir (char *directory)
{
...
int
mmap_string_ref (MMAPString * string)
{
	chash *ht;
	int r;
	chashdatum key;
...
void
mmap_string_set_ceil (size_t ceil)
{
...
static MMAPString *
mmap_string_realloc_file (MMAPString * string)
{
	char *data;
...
static MMAPString *
mmap_string_realloc_memory (MMAPString * string)
{
	char *tmp;
...
static MMAPString *
mmap_string_maybe_expand (MMAPString * string, size_t len)
{
	if (string->len + len >= string->allocated_len) {
		...
		string->allocated_len =
			nearest_power (1, string->len + len + 1);
...
mmap_string_sized_new (size_t dfl_size)
{
	...
	if (mmap_string_maybe_expand (string, MMAPSTRING_MAX (dfl_size, 2)) ==
	    NULL)
		return NULL;
...
MMAPString *
mmap_string_new_len (const char *init, size_t len)
{
	MMAPString *string;
...
MMAPString *
mmap_string_assign (MMAPString * string, const char *rval)
{
 ...
 **/
MMAPString *
mmap_string_set_size (MMAPString * string, size_t len)
{
	if (len >= string->allocated_len)
		if (mmap_string_maybe_expand (string, len - string->len) ==
		    NULL)
			return NULL;
...
mmap_string_insert_len (MMAPString * string,
			size_t pos, const char *val, size_t len)
{
...
MMAPString *
mmap_string_append (MMAPString * string, const char *val)
{
	return mmap_string_insert_len (string, string->len, val,
				       strlen (val));
}

MMAPString *
mmap_string_append_len (MMAPString * string, const char *val, size_t len)
{
	return mmap_string_insert_len (string, string->len, val, len);
}

MMAPString *
mmap_string_append_c (MMAPString * string, char c)
{
	return mmap_string_insert_c (string, string->len, c);
}

MMAPString *
mmap_string_prepend (MMAPString * string, const char *val)
{
	return mmap_string_insert_len (string, 0, val, strlen (val));
}

MMAPString *
mmap_string_prepend_len (MMAPString * string, const char *val, size_t len)
{
	return mmap_string_insert_len (string, 0, val, len);
}

MMAPString *
mmap_string_prepend_c (MMAPString * string, char c)
{
	return mmap_string_insert_c (string, 0, c);
}

MMAPString *
mmap_string_insert (MMAPString * string, size_t pos, const char *val)
{
	return mmap_string_insert_len (string, pos, val, strlen (val));
}

MMAPString *
mmap_string_insert_c (MMAPString * string, size_t pos, char c)
{
...
MMAPString *
mmap_string_erase (MMAPString * string, size_t pos, size_t len)
{
